  ...
  SUBROUTINE run_BMB_model_Favier2019_quadratic(      grid, ice, ocean, BMB)
    ...
    DO j = 1, grid%ny
      ...
      IF (ice%mask_shelf_a( j,i) == 1) THEN
        ...
        BMB%BMB_shelf( j,i) = -sec_per_year * C%BMB_Favier2019_quad_GammaT * ABS( dT) * dT * (seawater_density * cp_ocean / (ice_density * L_fusion))**2._dp
...
! ===================
  ...
  SUBROUTINE run_BMB_model_LADDIE(         grid,  BMB, region_name, time)
...
! ===================================================
  ...
  SUBROUTINE inverse_BMB_shelf_geometry( grid, ice, BMB, refgeo)
    ...
    DO j = 1, grid%ny
      ...
      IF (ice%mask_shelf_a( j,i) == 1) THEN
        ...
        BMB%BMB_shelf( j,i) = BMB%BMB_shelf( j,i) - ((ice%Hi_a( j,i) - refgeo%Hi( j,i)) / H0) - (ice%dHi_dt_a( j,i) / dHdt0)
