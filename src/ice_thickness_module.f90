...
CONTAINS
  ...
  SUBROUTINE calc_dHi_dt_explicit( grid, ice, SMB, BMB, dt, time)
    ...
    DO j = 1, grid%ny
      ...
      ! If calving exceeds the amount of remaining ice, transfer the rest to neighbouring interior cells
      IF (-dVi_calv( j,i) > Vi_available + dVi_MB( j,i)) THEN
        n = 0
      ...
      ! If this grid cell is a sub-grid calving front, don't allow for outfluxes until it is full
      IF (ice%mask_cf_a( j,i) == 1 .AND. &
          ice%mask_shelf_a( j,i) == 1 .AND. &
          ice%float_margin_frac_a( j,i) < 1._dp) THEN
