...
! == The no-ice mask, to prevent ice growth in certain areas
  ...
  SUBROUTINE initialise_mask_noice_GRL_remove_Ellesmere( grid, mask_noice)
    ...
    CALL oblique_sg_projection( pa_latlon(2), pa_latlon(1), grid%lambda_M, grid%phi_M, grid%beta_stereo, xa, ya)
    CALL oblique_sg_projection( pb_latlon(2), pb_latlon(1), grid%lambda_M, grid%phi_M, grid%beta_stereo, xb, yb)
