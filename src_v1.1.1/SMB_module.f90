...
CONTAINS
  ...
  SUBROUTINE run_SMB_model(            grid, ice, climate, time, SMB, mask_noice)
    ...
    DO j = 1, grid%ny
      ...
      IF ((ice%mask_ocean_Aa( j,i) == 1 .AND. ice%mask_shelf_Aa( j,i) == 0) .OR. mask_noice( j,i) == 1) SMB%AlbedoSurf( j,i) = albedo_water
      IF (ice%mask_ice_Aa(    j,i) == 1) SMB%AlbedoSurf( j,i) = albedo_ice
      ...
      DO m = 1, 12  ! Month loop
        ...
        IF ((ice%mask_ocean_Aa( j,i) == 1 .AND. ice%mask_shelf_Aa( j,i) == 0) .OR. mask_noice( j,i) == 1) SMB%Albedo( m,j,i) = albedo_water
        ...
        IF (ice%mask_ice_Aa( j,i) == 0 .OR. mask_noice( j,i) == 1) SMB%Refreezing( m,j,i) = 0._dp
  ...
  !      snowfrac = MAX(0._dp, MIN(1._dp, 0.5_dp   * (1 - ATAN((climate%T2m(vi,m) - T0) / 3.5_dp)  / 1.25664_dp)))
      ...
      IF (ice%mask_ice_Aa( j,i)==0) SMB%Refreezing_year( j,i) = 0._dp
