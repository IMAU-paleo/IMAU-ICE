...
framerate      = 5;
movie_filename = 'MISMIPplus_animation';
if exist(movie_filename,'file')
  delete(movie_filename)
end
VW = VideoWriter(movie_filename,'MPEG-4');
VW.FrameRate = framerate;
open(VW);

%% Read data
filename = 'MISMIPplus_ice1ra_5km_slid1/restart_ANT.nc';
...
H.Ax  = axes('parent',H.Fig,'units','pixels','position',[-100,-50,wa+25+100,ha+25+50]);
...
zdata = Hb;
surf('parent',H.Ax,'xdata',xdata,'ydata',ydata,'zdata',zdata,'facecolor',color,'facealpha',0.7,'edgecolor','none');
for i = 1:4:length(x)
...
% Make axes prettier
set(H.Ax,'xtick',[],'ytick',[],'ztick',[],'xlim',[0,800],'ylim',[-120,120],'zlim',[-800,2000],...
  'cameraposition',[0.3688   -0.1520    1.2474]*1e4);
...
fx = 0.5;
fy = 0.4;
H.Ax2 = axes('parent',H.Fig,'units','pixels','position',[25+wa*(1-fx),25+ha*(1-fy),wa*fx,ha*fy],...
  'xlim',[0,800],'ylim',[-800,2000],'fontsize',24,'xgrid','on','ygrid','on');
xlabel(H.Ax2,'x (km)')
ylabel(H.Ax2,'z (m)')
