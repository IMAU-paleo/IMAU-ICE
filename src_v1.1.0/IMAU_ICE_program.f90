...
!  ! TEST
!  REAL(dp), DIMENSION(:,:), POINTER :: A, B
!  INTEGER :: wA, wB, i, j, nx, ny, i1, i2, j1, j2
...
!  nx = 20000
!  ny = 5000
!  CALL allocate_shared_dp_2D( ny, nx, A, wA)
!  CALL allocate_shared_dp_2D( ny, nx, B, wB)
!  
...
!  DO i = i1, i2
!  DO j = 1, ny
!    A( j,i) = SQRT( REAL(i-5000,dp)**2 + REAL(j-5000,dp)**2)
!  END DO
!  END DO
!  CALL sync
!  tstop = MPI_WTIME()
!  IF (par%master) WRITE(0,*) ' test 1: dt = ', tstop - tstart
!  
!  ! test 2
!  tstart = MPI_WTIME()
!  DO i = 1, nx
...
!  CALL sync
!  tstop = MPI_WTIME()
!  IF (par%master) WRITE(0,*) ' test 2: dt = ', tstop - tstart
!  
!  ! test 1
!  tstart = MPI_WTIME()
!  DO i = i1, i2
