MODULE SMB_module
  ...
  USE parameters_module,               ONLY: T0, L_fusion, sec_per_year, pi
...
CONTAINS
  ...
  SUBROUTINE run_SMB_model( grid, ice, climate, time, SMB)
    ...
    DO j = 1, grid%ny
      ...
      IF (ice%mask_ocean_Aa( j,i) == 1) SMB%AlbedoSurf( j,i) = albedo_water
      ...
      DO m = 1, 12  ! Month loop
        ...
        IF (ice%mask_ocean_Aa( j,i)==1) SMB%Albedo( m,j,i) = albedo_water
