...
%% Read data
foldernames_perfect = {...
  'BIVMIP_B_perfect_40km',...
  'BIVMIP_B_perfect_20km',...
...
alpha_noicemask = 1.0;
image('parent',H.Ax(1,1),'xdata',perfect(3).x,'ydata',perfect(3).y,'cdata',ones(length(perfect(3).x),length(perfect(3).y),3),...
  'alphadata',double(perfect(3).Hi'<0.1)*alpha_noicemask,'alphadatamapping','none');
...
for j = 1:3
  ...
  image('parent',H.Ax(1,j+1),'xdata',inverted(j).x,'ydata',inverted(j).y,'cdata',ones(length(inverted(j).x),length(inverted(j).y),3),...
    'alphadata',double(inverted(j).Hi'<0.1)*alpha_noicemask,'alphadatamapping','none');
end
